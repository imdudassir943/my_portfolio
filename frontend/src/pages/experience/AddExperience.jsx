...
export default function AddExperience() {
  ...
  const fetchProfileData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Assuming profile returns a list and we take the first item
  ...
  // --- EFFECTS ---
  useEffect(() => {
    // 1. Get textual info from local storage
    ...
    // 2. Fetch the profile image from the DB
    fetchProfileData();
  ...
  return (
    <div className="pt-6 px-6 pb-12 relative">
      ...
      <h1 className="text-3xl font-bold mb-8 text-center text-white">Add Experience</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-lg max-w-3xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <label className="block">
            ...
            <input type="text" name="job_title" value={form.job_title} onChange={handleChange} className="w-full mt-1 px-4 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white focus:border-purple-500 outline-none transition" placeholder="Software Engineer" required />
          </label>

          <label className="block">
            <span className="text-gray-300 font-medium text-sm">Company</span>
            <input type="text" name="company" value={form.company} onChange={handleChange} className="w-full mt-1 px-4 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white focus:border-purple-500 outline-none transition" placeholder="Tech Solutions Inc." required />
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <label className="block">
            ...
            <input type="text" name="location" value={form.location} onChange={handleChange} className="w-full mt-1 px-4 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white focus:border-purple-500 outline-none transition" placeholder="New York, NY" />
          </label>
        ...
        <label className="block mb-6">
          ...
          <textarea name="description" value={form.description} onChange={handleChange} rows={5} className="w-full mt-1 px-4 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white focus:border-purple-500 outline-none transition" placeholder="Outline your key responsibilities and achievements..." />
        </label>

        <div className="flex justify-between mt-8 border-t border-slate-700 pt-6">
          <button type="button" onClick={() => navigate("/experience-view")} className="px-6 py-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-white transition shadow-md font-medium">
            Cancel
          </button>
